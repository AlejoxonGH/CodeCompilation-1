...
public class SpatialGrid : MonoBehaviour
{
    ...
    public void Unsubscribe(GridEntity ge)
    {
        var lastPos = lastPositions.ContainsKey(ge) ? lastPositions[ge] : Outside;

        if (lastPositions.ContainsKey(ge))
            lastPositions.Remove(ge);

        if (lastPos == Outside)
        {
            for (int i = 0; i < width; i++)
                for (int j = 0; j < height; j++)
                    if (buckets[i, j].Contains(ge))
                        buckets[i, j].Remove(ge);

            return;
        }

        if (buckets[lastPos.Item1, lastPos.Item2].Contains(ge))
            buckets[lastPos.Item1, lastPos.Item2].Remove(ge);
    }
